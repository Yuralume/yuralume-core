...
- **実世界ファクトの注入** — 祝日（`holidays`）、天気（Open-Meteo、キー不要）、キュレーション済み RSS ニュースが *事実* としてプロンプトに流れ、反応するかどうかは LLM が判断します。ハードコードされた振る舞いはありません。キャラクター自身が話題にしたニュースは元記事リンクとともに数日間チャットの文脈に残り、詳細を聞かれたら `web_fetch` で原文を読んでから答えます。
- **ストーリーツール** — キャラクターのストーリーアーク、複数キャラクターの fusion 短編、分岐ドラマ（VN 風）ジェネレーター。演じられたアークのビートは履歴として残り、完了したアークはマイルストーン記憶を残します。**「幕開け」**はスケジュールを待たずにプレイヤー自身が物語を引き出すボタンです。一度押すと通常のチャットスレッドの中にシーンフレーム（ナレーション＋キャラクターの最初の一言＋タイトル／場所／雰囲気）が開き、プレイヤーはこれまで通り自由に入力しつつ、毎ターン 2–3 個の任意の行動候補を受け取れます。ドラマチック・クエスチョンが解決したとモデルが判断した場合も、プレイヤーが自分で終えた場合も、放置してアイドルタイムアウト（既定 24 時間、`YURALUME_STORY_SCENE_IDLE_TIMEOUT_SECONDS`）した場合も、演じられた内容は正史として残ります。締めのナレーションがプレイヤーの行動を捏造することはありません。
...
| `KOKORO_OFFICIAL_CARD_CATALOG_URL` | 公式キャラクターカードの取得元。公式カードはリポジトリに同梱されなくなり、ホスト版・セルフホストを問わずすべてのデプロイが公開カタログから読み込みます（既定値 `https://app.yuralume.com/api/user/v1/public/official-cards`）。**送信される内容**：匿名の HTTP `GET` のみ。誰かがカードギャラリーを最初に開いたときに発行され、起動時には発行しません。アカウント・トークン・デプロイ識別子・バージョン・テレメトリのいずれも送りません。本アプリが自身のために行う唯一の外向き通信です。**無効化**：この変数を空にします。公式カード棚が消え、自分の `.lumecard`（`CHARACTER_CARD_PACK_DIR`）はそのまま使え、すでにインストール済みのキャラクターには一切影響がなく（インストール後は完全にローカルで、カタログへ再接続しません）、外部への接続は一切行われません。 |
| `CHARACTER_CARD_PACK_DIR` | 任意。ローカルの `.lumecard` ディレクトリ。既定は同梱の `src/kokoro_link/data/character_cards/`（現在は空で出荷）。ここに置いたカードは公式カードと並んで表示され、上のカタログを無効化した場合はここのカードだけが表示されます。 |
...
| `YURALUME_STORY_SCENE_IDLE_TIMEOUT_SECONDS` | 任意。放置された「幕開け」ストーリーシーンが自動的に締められ正史として残るまでのアイドル時間、デフォルト `86400`（24 時間）。この機能で設定可能な唯一の数値です——ペースにハードコードされた上限はありません。 |
...
同じプロバイダの接続を複数保持できます——例えば中継業者の API キーごとに使えるモデルが違うケースです。2 つ目以降の接続には**接続スラッグ**（詳細設定の項目）を入れてください。スラッグは model セレクターで選ぶ id の一部になり（`custom_openai_compatible__relay-b`）、各カードにその id が表示されます。空欄なら従来どおりのプリセット id のままなので、既存の環境には影響しません。対象は LLM・画像・動画で、TTS / embedding / 検索は引き続き同時に 1 つだけ有効（最終更新のものが稼働し、残りは standby）です。
