...
- **Real-world fact injection** — holidays (via `holidays`), weather (via Open-Meteo, no API key), and curated RSS news flow into the prompt as *facts only*; user profiles can store an editable coarse location that can be seeded from the user's login IP, and the LLM decides how to react, no hard-coded behaviours. News a character brought up itself stays in chat context for a few days *with its original link*, so a follow-up question is answered by fetching the source rather than guessing from a clipped summary.
- **Story tooling** — character story arcs, multi-character fusion short stories, and a branching-drama (VN-style) generator. Realized arc beats stay visible as history, and completed arcs leave milestone memories. **Start a Scene** lets the player pull the story instead of waiting for the schedule: one press opens a framed scene inside the ordinary chat thread (narration + the character's first line + a title/place/mood frame), the player roleplays freely with 2–3 optional action chips per turn, and the scene lands as canon whichever way it ends — the model judging the dramatic question resolved, the player ending it, or an idle timeout (default 24h, `YURALUME_STORY_SCENE_IDLE_TIMEOUT_SECONDS`). A wrap-up never invents player actions.
...
Want your own voice engine (GPT-SoVITS, XTTS, …)? Core speaks the same kind of small HTTP contract to a **Custom TTS Server** you run yourself — `GET /voices` + `POST /tts/synthesize`. See [`docs/CUSTOM_TTS_SERVER_SPEC.md`](docs/CUSTOM_TTS_SERVER_SPEC.md) for the full spec plus a minimal starter that wraps GPT-SoVITS, or open it in-app from **Admin → Developer docs**. Don't want to write a wrapper yourself? [`custom-tts-kokoro`](https://github.com/Yuralume/custom-tts-kokoro) is an official ready-made reference implementation wrapping the open-source Kokoro-82M model — one `docker run` and you're done. Prefer BYOK without running anything? Configure the built-in OpenAI TTS provider instead.
...
| `YURALUME_CLOUD_ENABLED` / `YURALUME_CLOUD_*` | Reserved for the official hosted cloud (managed deployments). Not applicable to self-host — leave unset. |
...
| `KOKORO_OFFICIAL_CARD_CATALOG_URL` | Where the official character cards come from. They are no longer shipped inside the repo, so every install — hosted or self-hosted — reads them from a public catalog (default `https://app.yuralume.com/api/user/v1/public/official-cards`). **What leaves your machine:** an anonymous HTTP `GET`, made the first time somebody opens the card gallery and never at startup — no account, no token, no deployment id, no version, no telemetry. It is the only outbound call Yuralume makes on its own behalf. **To turn it off:** set the variable to an empty value. The official shelf disappears, your own `.lumecard` packs (`CHARACTER_CARD_PACK_DIR`) still work, characters you already installed are entirely unaffected (an installed card is local and never contacts the catalog again), and nothing is contacted at all. |
| `CHARACTER_CARD_PACK_DIR` | Optional local `.lumecard` pack directory, defaulting to the bundled `src/kokoro_link/data/character_cards/` — which now ships empty. Drop cards here (or mount a folder) and they appear in the gallery alongside the official ones, or instead of them when the catalog above is switched off. |
...
You can keep several connections of the *same* provider preset — e.g. a relay vendor whose API keys each serve a different model line-up. Give every connection after the first one a **connection slug** (advanced field); it becomes part of the id you pick in the model selector (`custom_openai_compatible__relay-b`), and each card shows its own id. Leaving the slug blank keeps the plain preset id, so existing installs are unaffected. This applies to the LLM, image and video capabilities; TTS, embedding and search still mount one backend at a time (most recently updated wins, the rest stay on standby).
